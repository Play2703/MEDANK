...
3. **Medical Entity Recognition (NER & 20 Types) — HÍBRIDO**: O endpoint `/api/extract-entities` usa o `HybridNEREngine` (`src/core/ner/HybridNEREngine.ts`), que combina:
   - **Camada LOCAL** (`DictionaryNEREngine`, Aho-Corasick, dicionário de 13.980 termos + códigos CID-10 como sinônimos) — sempre disponível, offline, alta precisão, fonte de verdade para categorias e relações clínicas (gatilhos).
   - **Camada de IA** (`GeminiAiNerProvider`, plugável) — mapeia ~20 tipos clínicos para as 6 categorias do dicionário e resolve códigos **CID-10/SNOMED CT** reais. Ativa somente quando `GEMINI_API_KEY`/`VITE_GEMINI_API_KEY` está presente; sem chave, degrada para modo `local-only` (ainda 100% funcional).
   - O merge prioriza a camada local em sobreposição de span e enriquece entidades locais com os códigos resolvidos pela IA. Resultado armazenado em Dexie `chunkEntities` (DB v9).
