 ...
 * 2. OCR Local via Tesseract.js / WASM (100% local, 0 tokens).
...
import { OCRMode } from '../../core/exam_bank/services/LocalOCRService';
...
export const ExamQuestionSegmentationModal: React.FC<ExamQuestionSegmentationModalProps> = ({
  ...
  const handleSegment = async (customFile?: File | Blob, requestedOcrMode: OCRMode = 'local') => {
  ...
  const renderEngineBadge = () => {
    ...
    if (result.extractionMethod === 'local-ocr') {
      return (
        <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-semibold uppercase bg-purple-500/20 text-purple-300 border border-purple-500/30 flex items-center gap-1">
          <Cpu className="w-3 h-3" />
          OCR Local (100% Local / 0 Tokens)
        </span>
      );
    }
    ...
    if (result.extractionMethod === 'remote-ocr') {
      return (
        <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-semibold uppercase bg-amber-500/20 text-amber-300 border border-amber-500/30 flex items-center gap-1">
    ...
    return (
      <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-semibold uppercase bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center gap-1">
        <FileText className="w-3 h-3" />
        Camada de Texto Nativa
      </span>
    );
  };
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {isProcessing ? (
            ...
            <div className="py-16 text-center space-y-5 max-w-lg mx-auto">
              ...
              {/* Botões de Ação para Fallback de OCR */}
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                ...
                >
                  ...
                  <span>Executar OCR Local (Sem IA / 0 Tokens)</span>
                </button>
