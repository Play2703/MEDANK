...
Crie exatamente ${quantity} questões inéditas de múltipla escolha inspiradas na ${originLabel}, seguindo fielmente todas as regras e o material acima.`;
  ...
  // AI Basic Cycle Bridge Question Generation Endpoint (Fase 34 - Basic Cycle from Clinical Case)
  app.post("/api/generate-basic-cycle-from-clinical", async (req, res) => {
    try {
      const {
        clinicalQuestion,
        contextMaterial = "",
      ...
      if (!clinicalQuestion || (!clinicalQuestion.statement && typeof clinicalQuestion !== 'string')) {
        return res.status(400).json({ error: "clinicalQuestion é obrigatório para gerar questão de ciclo básico." });
      }

      const clinicalStatement = typeof clinicalQuestion === 'string' ? clinicalQuestion : clinicalQuestion.statement || '';
      const clinicalCommentary = typeof clinicalQuestion === 'string' ? '' : (
        typeof clinicalQuestion.commentary === 'string'
          ? clinicalQuestion.commentary
          : clinicalQuestion.commentary?.correta || ''
      );
      const clinicalSpecialty = typeof clinicalQuestion === 'string' ? 'Medicina' : clinicalQuestion.specialty || 'Medicina';
      const clinicalTopic = typeof clinicalQuestion === 'string' ? 'Geral' : clinicalQuestion.topic || 'Geral';
      const targetDiscipline = discipline || 'Ciências Básicas (Anatomia, Fisiologia, Bioquímica, Farmacologia)';

      const prompt = `Você é um professor titular de Ciências Básicas da Saúde (Anatomia Humana, Fisiologia Médica, Bioquímica Clínica, Histologia, Embriologia e Farmacologia Básica).

=== MATERIAL MÉDICO E CONHECIMENTO DE REFERÊNCIA (RAG / GRAFO DE CIÊNCIAS BÁSICAS) ===
${contextMaterial || 'Fundamentos de anatomia, fisiologia celular/sistêmica, vias metabólicas e farmacodinâmica de acordo com Guyton, Moore e Goodman & Gilman.'}
=== FIM DO MATERIAL ===

REGRAS RÍGIDAS DE ELABORAÇÃO E QUALIDADE:
1. FOCO EXCLUSIVO EM MECANISMO, ESTRUTURA OU FUNÇÃO BIOLÓGICA:
   - A nova questão DEVE avaliar a base científica fundamental: relações anatômicas (irrigação, inervação, drenagem, topografia), mecanismos fisiológicos/regulatórios (eixos hormonais, potenciais de ação, transporte de membrana, trocas gasosas), vias bioquímicas (enzimas, substratos) ou farmacodinâmica molecular (receptores, segundos mensageiros).
   - É EXPRESSAMENTE PROIBIDO criar uma questão de conduta médica clínica ("qual a conduta imediata", "qual o diagnóstico do caso", "qual antibiótico prescrever").
2. ADAPTAÇÃO A PARTIR DO CONCEITO (SEM COPIAR A VINHETA CLÍNICA):
   - A questão clínica de referência serviu unicamente para identificar qual órgão, via biológica ou fármaco foi abordado.
   - NUNCA copie o enunciado da questão clínica nem reproduza a história da doença do paciente. Crie uma pergunta direta e objetiva sobre o mecanismo ou anatomia subjacente.
3. ESTRUTURA DAS ALTERNATIVAS E COMENTÁRIO:
   - Crie exatamente 4 alternativas (A, B, C, D) simétricas e tecnicamente plausíveis com apenas 1 alternativa indiscutivelmente correta.
   - Inclua um comentário estruturado contendo a justificativa detalhada e explicação de cada distrator.

Retorne EXCLUSIVAMENTE em formato JSON VÁLIDO (sem texto extra, sem bloco de código markdown):
[
  {
    "statement": "Enunciado direto e objetivo testando o mecanismo fisiológico, estrutura anatômica ou base farmacológica...",
    "clinicalContext": "Correlação breve com o conceito biológico",
    "correctAnswerText": "Texto da resposta correta",
    "correctAnswerExplanation": "Justificativa direta da base científica correta",
    "commentary": {
      "correta": "Explicação completa e aprofundada do mecanismo/estrutura biológica embasada na literatura...",
      "porOpcao": {
        "A": "Explicação do erro da alternativa A...",
        "B": "Explicação do erro da alternativa B...",
        "C": "Explicação do erro da alternativa C...",
        "D": "Explicação do erro da alternativa D..."
      },
      "correlacaoClinica": "Síntese da correlação entre a ciência básica e a aplicação na prática médica."
    },
    "references": ["Guyton & Hall - Tratado de Fisiologia Médica", "Moore - Anatomia Orientada para a Clínica"],
    "tags": ["Ciclo Básico", "${clinicalSpecialty}", "${clinicalTopic}"],
...
=== QUESTÃO CLÍNICA DE REFERÊNCIA (UTILIZE APENAS PARA EXTRAIR O CONCEITO SUBJACENTE) ===
Enunciado Original: ${clinicalStatement}
${clinicalCommentary ? `Gabarito/Conceito Original: ${clinicalCommentary}` : ''}
=== FIM DA QUESTÃO DE REFERÊNCIA ===

COMANDO DE GERAÇÃO:
Crie exatamente 1 questão inédita de múltipla escolha focada no ciclo básico (${targetDiscipline}) inspirada no conceito subjacente acima.`;

      const result = await parallelAIService.generateQuestionsParallel(prompt, undefined, 0.35);

      if (!result.success || !result.mainData) {
        throw new Error(result.error || "Falha na geração de questão de ciclo básico.");
      }
      ...
      console.error("Erro ao gerar questão de ciclo básico a partir de clínica:", error);
      return res.status(500).json({
        error: "Falha ao gerar questão de ciclo básico a partir da questão clínica.",
        details: error.message || String(error),
      });
    }
  });
